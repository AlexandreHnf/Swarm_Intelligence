--[[
Floor color in room 0: 0.418263
Num objects in room 0: 3 evaluated: 0.1
Room 0 evaluation: 0.259131
Floor color in room 1: 0.135462
Num objects in room 1: 10 evaluated: 0.8
Room 1 evaluation: 0.467731
Floor color in room 2: 0.2668
Num objects in room 2: 8 evaluated: 0.6
Room 2 evaluation: 0.4334
Floor color in room 3: 0.112759
Num objects in room 3: 4 evaluated: 0.2
Room 3 evaluation: 0.15638
...
PROBLEMS : 
=========================== SOLVED
- le align des fois est infini -> sol = le faire bouger un peu apres le delai
- les robots sont bloqués derrière les objets lors de leur exploration => sol : limiter leur entrée qd ils entrent dans une piece et detecter une led d'entrée sans limite de distance lorsqu'il est dans une piece
- si on fait en sorte que les premiers robots qui entrent dans une piece sont scouts,  il y en a plus que 4 et du coup pour la synchro est difficile => sol = assigner une piece par scout et que 4 scouts en tout
- si y'a pas de scout et que tout le monde explore et dès qu'il entre dans une room, il reste avec une probabilitté proportionnelle a la qualité de la room, puis leave avec une probabilité proportionnelle au nombre de robots dans la salle, ca marche pas parce que le nombre de robots est random et pas stable. Pas de convergence. => sol = que 4 scouts
- 

=========================== UNSOLVED
- certain robot sont coincés a l'entrée (dans une room) et donc peuvent pas se synchro
- qd les robots foncent dans la piece ultime, ils se bloquent a l'entrée
- un robot peut etre bloqué par d'autres dans la piece principale du coup jamais aller en mode SYNCHRO => sol : pouvoir se synchro meme en mode explorer
- des fois un scout se fait pousser dans une piece et du coup des types en synchro restent en synchro pdt que les autres ont décidé.
- des fois les autres scouts a la fin, poussent un follower dans une piece et du coup le follower est perdu. => sol = rallier les autres scouts a la meme piece.
   ...
   -- ================================== EXPLORE =====================================
	...
	elseif current_state == ROOM then 
		...
		if is_in_room then 
			if (not is_scout) or (is_scout and Q == -1) then 
				Q = get_room_quality() -- replace room quality in memory
			end
	...
	elseif current_state == SYNCHRO then 
		...
		if is_in_room then -- if stopped in some way in a room, continue exploring
			...
			waiting_robots = nb_waiting_robots() -- nb of brown and white

			-- the scout senses 3 other scouts and 16 followers
			if is_scout and waiting_robots[1] == 3 and waiting_robots[2] == 16 then 
				current_wiggle_time = Q*100 -- timer proportional to the quality
				current_state = DECIDE 

			-- the follower senses 4 scouts and 15 other followers
			elseif not is_scout and waiting_robots[1] == 4 and waiting_robots[2] == 15 then  
				current_state = DECIDE
			end
		end
	...
	elseif current_state == DECIDE then 
		...
		if is_scout then  			-- SCOUT 
			if waiting_robots[1] == 0 then -- it means he is the only remaining
		...
		elseif not is_scout then		  -- FOLLOWER
			if waiting_robots[1] == 0 then -- only one scout wiggling remaining
	...
	elseif current_state == STOP then
		...
		-- if is_in_room then current_state = EXPLORE end -- TEMPORARY
...
function get_room_quality() -- returns quality of a room based on ground color and objects
	...
	for i = 1, nb_led_sensed do 
		led = robot.colored_blob_omnidirectional_camera[i]
		if (led.color.green == 255 and led.distance <= 250) then -- green
...
function nb_waiting_robots()
	nb_scouts_sensed = 0
	nb_followers_sensed = 0
	nb_led_sensed = #robot.colored_blob_omnidirectional_camera
	for i = 1, nb_led_sensed do 
		led = robot.colored_blob_omnidirectional_camera[i].color
		if led.red == 255 and led.green == 255 and led.blue == 255 then -- white
			nb_followers_sensed = nb_followers_sensed + 1
		elseif led.red == 128 and led.green == 0 and led.blue == 0 then -- brown
			nb_scouts_sensed = nb_scouts_sensed + 1
		end
	end
	-- log("nb scouts_sensed " .. nb_scouts_sensed)
	-- log("nb followers sensed " .. nb_followers_sensed)
	return {nb_scouts_sensed, nb_followers_sensed}
end
